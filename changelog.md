...
* _ImageViewer plugin_: New actions: **I** - show full EXIF metadata using `exiftool`; **G** - choose a map service and open the GPS location from the current file's EXIF metadata in a browser; **T** - exit and move the panel cursor to the currently viewed file.
