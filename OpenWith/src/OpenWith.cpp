...
namespace openwith
{
											   ...
											   const std::vector<CandidateContextLocation>& locations)
	{
		...
		const int max_label_cell_width = GetMaxLabelCellWidth(details);
		const int details_dlg_min_width = std::max({standard_buttons_span, locations_span, max_label_cell_width + 11}) + DlgLayout::H_TOTAL_OVERHEAD;
		...
		const int label_end_x = DlgLayout::H_SIDE_OVERHEAD + max_label_cell_width - 1;
		const int edit_start_x = DlgLayout::H_SIDE_OVERHEAD + max_label_cell_width + 1;
	...
	int Plugin::CalculateVisibleCellWidth(std::wstring_view text)
	{
